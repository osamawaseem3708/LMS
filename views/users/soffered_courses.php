...
<div class="wrapper rounded bg-white" id="update_course" style="width: 90%">
    ...
    <div class="h3" style="padding: 2%;text-align: center">Offered Course Form</div>

    <?php if (Yii::$app->session->hasFlash('student_Enrollment')): ?>
        <div class="alert alert-success alert-dismissable" id="update_div">
            ...
            <h4><i class="icon fa fa-check"></i>Enrolled!</h4>
            <?= Yii::$app->session->getFlash('student_Enrollment') ?>
        </div>
    <?php endif; ?>
    ...
    <?php if (Yii::$app->session->hasFlash('limit_out')): ?>
        <div class="alert alert-danger alert-dismissable" id="update_div">
            <button aria-hidden="true" data-dismiss="alert" onclick="closediv()" class="close" type="button">×</button>
            <h4><i class="icon fa fa-check"></i>Warning!</h4>
            <?= Yii::$app->session->getFlash('limit_out') ?>
        </div>
    <?php endif; ?>
...
<!--    data-bs-toggle="modal" data-bs-target="#staticBackdrop"      -->
        ...
        <td><a data-bs-toggle="modal" data-bs-target="#staticBackdrop" class="btn btn-primary" style="color: white" aria-hidden="true" onclick="gettablerow('<?php echo $corse['cname']; ?>','<?php echo $corse['batch']; ?>'
                    ...
                    '<?php echo $corse['cid']; ?>','<?php echo $corse['offer_semester']; ?>','<?php echo $corse['teacher_id']; ?>'
                    ,'<?php echo $corse['fname']." ".$corse['lname']; ?>')">Enroll</a>  &nbsp;
...
<!--                Model Start-->
                <form method="post">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mt-md-0 mt-3"> <label>Teacher Name</label>
                            <input type="text" class="form-control" disabled id="tname" name="tname" required> </div>
                    </div>
                    <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mt-md-0 mt-3"> <label>Course Credit Hours</label>
                            ...
                            <input type="hidden" id="tid" required name="tid">
                            <input type="hidden" id="course_name" required name="course_name">
                        </div>
...
<script>
    function gettablerow(name,batch,category,description,credit_hours,status,cid,offer_semester,teacher_id,teachername){
    console.log("name= "+name+" batch= "+batch+" category= "+category+" description= "+description+" credit hours= "+credit_hours+" status= "+status+" Teacher Id= "+teacher_id);
        document.getElementById('cname').value = name;
        document.getElementById('course_name').value = name;
        document.getElementById('tname').value = teachername;
        ...
        document.getElementById('tid').value = teacher_id;
